...
inline byte sendInParticularBuf(unsigned long id, unsigned char * buf, byte bufferIdx, byte ext=0, byte msgLen=8){
  return CAN.trySendMsgBuf(id, ext, 0, msgLen, buf, bufferIdx);
}
